...
import rospy
import actionlib
from custom_msgs.msg import AcousticsGuessGoal, AcousticsGuessAction, AcousticsProcessingGoal, AcousticsProcessingAction, MessageTypeFeedback, MessageTypeResult, MessageTypeAction
import sys
import os
...
class ServerClientThing:
    ...
    def __init__(self):
        ...
        self.server = actionlib.SimpleActionServer(self.ACTION_NAME, MessageTypeAction, self.execute, False)
        self.server.start()
        rospy.spin()


    def publish_feedback(self, stage):
        feedback = MessageTypeFeedback()
        feedback.curr_stage = stage;
        feedback.total_stage = 2
        self.server.publish_feedback(feedback)

    def publish_result(self, processing_result):
        result = MessageTypeResult()
