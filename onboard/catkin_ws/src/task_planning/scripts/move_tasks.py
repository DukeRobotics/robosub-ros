...
@task
async def move_to_pose_global(self: Task, controls: ControlsInterface, state: StateInterface, pose: Pose) -> \
...
@task
async def move_to_pose_local(self: Task, controls: ControlsInterface, state: StateInterface, pose: Pose) -> \
        Task[None, Optional[Pose], None]:
    """
    ...
    return await coroutine_utils.transform(
        move_to_pose_global(controls, state, global_pose, parent=self),
        send_transformer=lambda p: geometry_utils.local_pose_to_global(state.tfBuffer, p) if p else p)


@task
async def move_with_velocity(self: Task, controls: ControlsInterface, state: StateInterface, twist: Twist) -> \
...
@task
async def move_with_power_for_seconds(self: Task, controls: ControlsInterface, power: Twist, seconds: float) -> \
        Task[None, Optional[Twist], None]:
    """
...
@task
async def hold_position(self: Task, controls: ControlsInterface, state: StateInterface) -> Task[bool, None, None]:
    """
