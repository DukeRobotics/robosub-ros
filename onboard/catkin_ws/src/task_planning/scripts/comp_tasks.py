...
async def after_buoy_task(self: Task):
    ...
    found_bins = await let_search_for_bin_turtlesim_style_because_why_not(parent=self)
...
async def coin_flip(self: Task, depth_level=0.7) -> Task[None, None, None]:
    ...
    def get_yaw_correction():
        ...
        return orig_imu_euler_angles[2] - cur_imu_euler_angles[2]  # delete this line to go back
        ...
        rospy.loginfo('coinflip: desired_yaw='+str(desired_yaw))
        return desired_yaw
...
# @task
# async def align_path_marker(self: Task, direction: int = 1) -> Task[None, None, None]:
#     """
...
#     async def correct_yaw():
#         yaw_correction = calculate_yaw_correction()
...
#     await correct_depth()
#     await correct_yaw()
#     rospy.loginfo("Path marker aligned")
...
async def align_path_marker(self: Task, direction=1) -> Task[None, None, None]:
    ...
    rospy.loginfo(f"Now aligning path marker in frame...")
...
@task
async def path_marker_to_bin(self: Task, maximum_distance: int = 6):
    DEPTH_LEVEL = State().orig_depth - 0.5
    AREA_THRESHOLD = 1000
    LATENCY_THRESHOLD = 1
    ...
    async def move_to_bins():
        ...
        while not is_receiving_bin_data("bin_red", bin_red_time) or not is_receiving_bin_data("bin_blue", bin_blue_time):
...
# @task
# async def dead_reckoning_path_marker_to_bin(self: Task, maximum_distance: int = 6):
#     DEPTH_LEVEL = State().orig_depth - 0.5
#     AREA_THRESHOLD = 1000
#     LATENCY_THRESHOLD = 1
...
#         yaw_correction = CV().cv_data["global_path_marker_angle"] - euler_angles[2]
#         rospy.loginfo(f"Yaw correction: {yaw_correction}")
...
@task
async def let_search_for_bin_turtlesim_style_because_why_not(self: Task) -> Task[None, None, None]:
    DEPTH_LEVEL = State().orig_depth - 0.5
    AREA_THRESHOLD = 1000
    LATENCY_THRESHOLD = 1
...
async def octagon_task(self: Task, direction: int = 1) -> Task[None, None, None]:
    ...
    async def move_to_pink_bins():
        ...
        while not is_receiving_pink_bin_data(latest_detection_time) and not moved_above:
            ...
            if not moved_above:
                await yaw_to_cv_object('bin_pink_front', direction=direction, yaw_threshold=math.radians(15), depth_level=0.9, parent=Task.MAIN_ID)
    ...
    await move_tasks.move_to_pose_local(geometry_utils.create_pose(0, 0, State().orig_depth - State().depth, 0, 0, 0), timeout=10, parent=self)
