...
# Code written during the summer 2022 competition
# Largely jank and unhelpful, but being stored here just in case
# As a justification, we had some issues with SMACH apparently, so we just went with what worked
# Previously located in controls/test_state_publisher.py
# Woe, legacy code be upon ye
import rospy
...
class JankCompCode:
    ...
    def recalculate_local_pose(self):
        self.desired_pose_transformed = self.transform_pose(
...
def main():
    # JankCompCode().gate_move()
    JankCompCode().semifinal_sunday_v1()
    # JankCompCode().publish_desired_pose_local()
    # JankCompCode().move_to_pos_and_stop(3,0,-1.2)
    # JankCompCode().publish_desired_twist()
    # JankCompCode().test_yaw()
    # JankCompCode().move_to_pos_and_stop(8,0,-1)
    # JankCompCode().move_to_yaw_and_stop(0,0,1,0) #submerges as well
    # print("Done with yaw")
    # JankCompCode().move_to_pos_and_stop(2,0,-0.5)
    print("Done with x")

    # JankCompCode().semifinal_sunday_v1()
    # JankCompCode().test_comm_cv()
