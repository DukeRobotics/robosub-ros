...
from move_tasks import (
    AllocateVelocityLocalTask,
    HoldPositionTask,
    MoveToUserDataPoseLocalTask,
    MoveToPoseLocalTask,
)
...
class BuoyTask(smach.StateMachine):
        ...
        with self:
            for i in range(len(self.targets)):
                smach.StateMachine.add(
                    f"CHOOSE_ROTATE_DIR_{i}",
                    cv_tasks.SpinDirectionTask(
                        self.targets[i], self.CENTER_TOLERANCE, cv
                    ),
                    transitions={
                ...
                smach.StateMachine.add(
                    f"ROTATE_LEFT_{i}",
                    AllocateVelocityLocalTask(
                        0, 0, 0, 0, 0, self.ROTATE_SPEED, controls
                    ),
                    transitions={"done": f"CHOOSE_ROTATE_DIR_{i}"},
                )
                smach.StateMachine.add(
                    f"ROTATE_RIGHT_{i}",
                    AllocateVelocityLocalTask(
                ...
                smach.StateMachine.add(
                    f"BUOY_CENTERED_{i}",
                    HoldPositionTask(controls),
                    transitions={"done": f"BUOY_VALID_{i}"},
                )
                smach.StateMachine.add(
                    f"BUOY_VALID_{i}",
                    cv_tasks.ObjectCoordsTask(self.targets[i], cv),
                    transitions={
                ...
                smach.StateMachine.add(
                    f"BUOY_POSE_{i}",
                    task_utils.PointToPoseTask(controls),
                    transitions={"done": f"BUOY_TOO_CLOSE_{i}"},
                )
                smach.StateMachine.add(
                    f"BUOY_TOO_CLOSE_{i}",
                    task_utils.LambdaTask(
                        self.check_too_close, ["close", "far"], input_keys=["pose"]
                    ),
                    transitions={
                ...
                smach.StateMachine.add(
                    f"APPROACH_BUOY_{i}",
                    MoveToUserDataPoseLocalTask(controls, listener),
                    transitions={
                ...
                smach.StateMachine.add(
                    f"MOVE_BACK_{i}",
                    MoveToPoseLocalTask(-2, 0, 0, 0, 0, 0, controls, listener),
                    transitions={
                        "continue": f"MOVE_BACK_{i}",
                        "done": f"CHOOSE_ROTATE_DIR_{i+1}"
    ...
    def check_too_close(self, userdata):
        if (
            userdata.pose.position.x**2
            + userdata.pose.position.y**2
            + userdata.pose.position.z**2
            > self.TOO_CLOSE_TO_BUOY
        ):
            return "far"
        return "close"
