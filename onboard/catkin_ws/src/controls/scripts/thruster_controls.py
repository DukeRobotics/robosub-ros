...
class ThrusterController:
    ...
    Attributes:
        ...
        thruster_allocs_pub: The ROS publisher that publishes thruster allocations used by downstream packages
    ...
    def __init__(self):
        ...
        self.thruster_allocs_pub = rospy.Publisher(self.ROBOT_PUB_TOPIC, ThrusterAllocs, queue_size=3)
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            if self.ROBOT_PUB_TOPIC in [pub[0] for pub in pubs]:
                if not self.enabled:
                    i8_t_allocs = ThrusterAllocs()
                    i8_t_allocs.allocs = np.zeros(8).astype(float)
                    self.thruster_allocs_pub.publish(i8_t_allocs)

                if self.enabled:
                    self._scale_thruster_allocs()
                    i8_t_allocs = ThrusterAllocs()
                    i8_t_allocs.allocs = (self.t_allocs * self.POWER_SCALING_FACTOR).astype(float)
                    self.thruster_allocs_pub.publish(i8_t_allocs)

            rate.sleep()

    def _scale_thruster_allocs(self):
        """Scales thruster allocs according to a custom algorithm. Doesn't scale if all allocations are 0.

        TODO: Determine if our scaling algorithm is valid. Currently, we first scale the max thruster allocation to the
        max PID control effort. Then we clamp values between -1 and 1 and multiply by max thruster alloc.
        """
