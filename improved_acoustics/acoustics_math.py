...
def data_to_pdiff(data, freq, pingc, fft_w_size, large_window_portion, fs):
    ...
    # plt.plot(plist_all[0])
    # plt.show()
    # plt.plot(mlist_all[0])
    # plt.show()
...
def plot_3d_locations(xLocations, yLocations, zLocations, guess):
    fig = plt.figure()
    ax = fig.add_subplot(111, projection='3d')
    for i in range(len(xLocations)):
        norm = np.linalg.norm([xLocations[i], yLocations[i], zLocations[i]])
        ax.plot([0, xLocations[i] / norm * 10], [0, yLocations[i] / norm * 10], [0, zLocations[i] / norm * 10])
    ax.scatter([guess[0]], [guess[0]], [guess[2]])
