...
sim_objectspecialproperty_detectable_all = sim_objectspecialproperty_detectable_ultrasonic | \
...
sim_message_ui_button_state_change = 0  # a UI button slider etc. changed (due to a user's action). aux[0]=UI handle
# aux[1]=button handle aux[2]=button attributes aux[3]=slider position (if slider)
...
sim_message_eventcallback_instancepass = 0x10d  # Called once every main application loop pass. auxiliaryData[0]
# contains event flags of events that happened since last time
...
sim_message_eventcallback_scripticondblclick = 0x11c  # scipt icon was double clicked.  (aux[0]=object handle
# associated with script set replyData[0] to 1 if script should not be opened)
...
sim_message_eventcallback_keypress = 0x120  # a key was pressed while the focus was on a page (aux[0]=key aux[1]=ctrl
# and shift key state)
sim_message_eventcallback_modulehandleinsensingpart = 0x121  # called when simHandleModule in Lua is called with
# argument true
...
sim_message_eventcallback_menuitemselected = 0x124  # auxiliaryData[0] indicates the handle of the item
# auxiliaryData[1] indicates the state of the item
...
sim_message_eventcallback_rmlposition = 0x12b  # the command simRMLPosition was called. The appropriate plugin should
# handle the call
sim_message_eventcallback_rmlvelocity = 0x12c  # the command simRMLVelocity was called. The appropriate plugin should
# handle the call
...
sim_dlgstyle_dont_center = 32  # can be combined with one of above values. Only with this flag can the position of
# the related UI be set just after dialog creation
...
sim_drawing_trianglepoints = 3  # 6 values per point (3 for triangle position 3 for triangle normal vector) (triangle
# size in meters)
...
sim_drawing_vertexcolors = 0x00040  # +3 values per vertex (each vertex has its own ambient color (rgb values). Only
# for sim_drawing_lines (+6) and for sim_drawing_triangles(+9)). Mutually exclusive with sim_drawing_itemcolors
...
sim_drawing_followparentvisibility = 0x00800  # if the object is associated with a scene object then it follows that
# visibility otherwise it is always visible
...
sim_drawing_emissioncolor = 0x10000  # When used in combination with sim_drawing_itemcolors or
# sim_drawing_vertexcolors then the specified colors will be for the emissive component
sim_drawing_facingcamera = 0x20000  # Only for trianglepoints quadpoints discpoints and cubepoints. If specified the
# normal verctor is calculated to face the camera (each item data requires 3 values less)
sim_drawing_overlay = 0x40000  # When specified objects are always drawn on top of "regular objects"
sim_drawing_itemtransparency = 0x80000  # +1 value per item (each item has its own transparency value (0-1)). Not
# compatible with sim_drawing_vertexcolors
...
sim_banner_followparentvisibility = 0x00010  # if the object is associated with a scene object then it follows that
# visibility otherwise it is always visible
sim_banner_clickselectsparent = 0x00020  # if the object is associated with a scene object then clicking the banner
# will select the scene object
sim_banner_clicktriggersevent = 0x00040  # if the banner is clicked an event is triggered (
# sim_message_eventcallback_bannerclicked and sim_message_bannerclicked are generated)
sim_banner_facingcamera = 0x00080  # If specified the banner will always face the camera by rotating around the
# banner's vertical axis (y-axis)
sim_banner_fullyfacingcamera = 0x00100  # If specified the banner will always fully face the camera (the banner's
# orientation is same as the camera looking at it)
sim_banner_backfaceculling = 0x00200  # If specified the banner will only be visible from one side
sim_banner_keepsamesize = 0x00400  # If specified the banner will always appear in the same size. In that case size
# represents the character height in pixels
sim_banner_bitmapfont = 0x00800  # If specified a fixed-size bitmap font is used. The text will also always fully
# face the camera and be right
# to the specified position. Bitmap fonts are not clickable


# particle objects following are mutually exclusive
sim_particle_points1 = 0  # 6 values per point (pt1 and pt2. Pt1 is start position pt2-pt1 is the initial velocity
# vector). i
# Point is 1 pixel big. Only appearance is a point internally handled as a perfect sphere
sim_particle_points2 = 1  # 6 values per point. Point is 2 pixel big. Only appearance is a point internally handled
# as a perfect sphere
sim_particle_points4 = 2  # 6 values per point. Point is 4 pixel big. Only appearance is a point internally handled
# as a perfect sphere
...
# following can be or-combined
sim_particle_respondable1to4 = 0x0020  # the particles are respondable against shapes (against all objects that have
# at least one bit 1-4 activated in the global respondable mask)
sim_particle_respondable5to8 = 0x0040  # the particles are respondable against shapes (against all objects that have
# at least one bit 5-8 activated in the global respondable mask)
sim_particle_particlerespondable = 0x0080  # the particles are respondable against each other
sim_particle_ignoresgravity = 0x0100  # the particles ignore the effect of gravity. Not compatible with
# sim_particle_water
...
sim_particle_emissioncolor = 0x4000  # When used in combination with sim_particle_itemcolors then the specified
# colors will be for the emissive component
sim_particle_water = 0x8000  # the particles are water particles (no weight in the water (i.e. when z<0)). Not
# compatible with sim_particle_ignoresgravity
...
# Navigation and selection modes with the mouse. Lower byte values are mutually exclusive upper byte bits can be
# combined
...
simx_headeroffset_scene_id = 15  # 1 simxUShort. Generated by the server. A unique ID identifying the scene currently
# displayed
simx_headeroffset_server_state = 17  # 1 byte. Generated by the server. Bit coded 0 set --> simulation not stopped 1
# set --> simulation paused 2 set --> real-time switch on 3-5 edit mode type (0=no edit mode 1=triangle 2=vertex
# 3=edge 4=path 5=UI)
...
simx_cmdheaderoffset_full_mem_size = 4  # 1 simxInt. Generated by the client or server. The full buffer size of the
# command (applies to split chunks).
simx_cmdheaderoffset_pdata_offset0 = 8  # 1 simxUShort. Generated by the client or server. The amount of data that is
# part of the command identification.
simx_cmdheaderoffset_pdata_offset1 = 10  # 1 simxInt. Generated by the client or server. The amount of shift of the
# pure data buffer (applies to split chunks).
simx_cmdheaderoffset_cmd = 14  # 1 simxInt. Generated by the client (and used in a reply by the server). The command
# combined with the operation mode of the command.
simx_cmdheaderoffset_delay_or_split = 18  # 1 simxUShort. Generated by the client or server. The amount of delay in
# ms of a continuous command or the max. pure data size to send at once (applies to split commands).
simx_cmdheaderoffset_sim_time = 20  # 1 simxInt. Generated by the server. The simulation time (in ms) when the
# command was executed (or 0 if simulation is not running)
simx_cmdheaderoffset_status = 24  # 1 byte. Generated by the server. (1 bit 0 is set --> error in function execution
# on server side). The client writes bit 1 if command cannot be overwritten
simx_cmdheaderoffset_reserved = 25  # 1 byte. Not yet used

# Regular operation modes
simx_opmode_oneshot = 0x000000  # sends command as one chunk. Reply will also come as one chunk. Doesn't wait for the
# reply.
simx_opmode_blocking = 0x010000  # sends command as one chunk. Reply will also come as one chunk. Waits for the reply
# (_REPLY_WAIT_TIMEOUT_IN_MS is the timeout).
simx_opmode_oneshot_wait = 0x010000  # sends command as one chunk. Reply will also come as one chunk. Waits for the
# reply (_REPLY_WAIT_TIMEOUT_IN_MS is the timeout).
...
simx_opmode_oneshot_split = 0x030000  # sends command as several chunks (max chunk size is x bytes where x can be
# _MIN_SPLIT_AMOUNT_IN_BYTES-65535. Just add x to opmode_oneshot_split). Reply will also come as several chunks.
# Doesn't wait for the reply.
simx_opmode_continuous_split = 0x040000
simx_opmode_streaming_split = 0x040000  # sends command as several chunks (max chunk size is x bytes where x can be
# _MIN_SPLIT_AMOUNT_IN_BYTES-65535. Just add x to opmode_continuous_split). Command will be stored on the server and
# always executed. A reply will be sent continuously each time as several chunks. Doesn't wait for the reply.

# Special operation modes
simx_opmode_discontinue = 0x050000  # removes and cancels all commands stored on the client or server side (also
# continuous commands)
simx_opmode_buffer = 0x060000  # doesn't send anything but checks if a reply for the given command is available in
# the input buffer (i.e. previously received from the server)
simx_opmode_remove = 0x070000  # doesn't send anything and doesn't return any specific value. It just erases a
# similar command reply in the inbox (to free some memory)
...
simx_return_split_progress_flag = 0x000010  # previous similar command not yet fully processed (applies to
# opmode_oneshot_split operation modes)
...
simx_error_split_progress_flag = 0x000010  # previous similar command not yet fully processed (applies to
# opmode_oneshot_split operation modes)
